...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_IdImagesForLongCat(),
        ]
        self.post_units = [
...
class WanVideoUnit_IdImagesForLongCat(PipelineUnit):
    """
    Pipeline unit for processing reference images for LongCat consistency I2V.
    Encodes 1-3 reference images into latents for concatenation at sequence end.
    """
    def __init__(self):
        super().__init__(
            input_params=("id_images",),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, id_images):
        """
        Process reference images and encode them into latents.
        
        Args:
            pipe: WanVideoPipeline instance
            id_images: Single PIL image or list of images (1-3 images)
        
        Returns:
            id_latents: Tensor of shape [B, C, N_ref, H, W], where N_ref is number of images
        """
        ...
        if not isinstance(id_images, list):
            id_images = [id_images]
        
        if len(id_images) > 3:
            id_images = id_images[:3]
        
        id_latents_list = []
        for img in id_images:
            img_as_video = pipe.preprocess_video([img])
            img_latent = pipe.vae.encode(img_as_video, device=pipe.device)
            img_latent = img_latent.to(dtype=pipe.torch_dtype, device=pipe.device)
            id_latents_list.append(img_latent)
        
        id_latents = torch.cat(id_latents_list, dim=2)
        
        return {"id_latents": id_latents}
...
def model_fn_longcat_video(
    ...
    """
    Forward inference function for LongCat-Video model.
    
    Supports two functionalities:
    1. Long video continuation (longcat_latents): Replace condition frames at the beginning
    2. Consistency I2V (id_latents): Concatenate reference images at sequence end with position encoding offset
    
    Args:
        dit: LongCat-Video DiT model
        latents: Video latents with shape [B, C, T, H, W]
        timestep: Timestep
        context: Text encoding
        longcat_latents: Condition frame latents for long video continuation
        id_latents: Reference image latents for consistency I2V with shape [B, C, N_ref, H, W]
        use_gradient_checkpointing: Whether to use gradient checkpointing
        use_gradient_checkpointing_offload: Whether to offload gradient checkpointing to CPU
    
    Returns:
        output: Predicted noise with shape [B, C, T, H, W] (excluding reference images)
    """
    ...
    num_id_latents = 0
    if id_latents is not None:
        num_id_latents = id_latents.shape[2]
        latents = torch.cat([latents, id_latents], dim=2)
